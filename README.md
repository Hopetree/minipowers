...
项目文档初始化技能。评估项目规模，推荐需要创建的设计文档（PRD、TDD、ERD、API、SDD、项目计划），与用户协作逐一撰写。通过 `docs/review-state.json` 文档注册表与 syncing-docs 联动。
...
`bootstrapping-docs` 和 `syncing-docs` 协同工作，覆盖文档全生命周期：

1. **bootstrapping-docs** — 创建初始文档体系，注册到 `docs/review-state.json`
2. **syncing-docs** — 代码变更时审查差异，自动识别受影响的文档并同步更新
