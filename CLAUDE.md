...
- `review-and-sync` — Code review with automated document impact analysis; reviews git diffs, produces AI-oriented review reports, identifies affected project docs, and syncs them
...
`bootstrapping-docs` and `review-and-sync` share a document registry at `docs/review-state.json`:

- **bootstrapping-docs** creates the initial document set and registers each document
- **review-and-sync** keeps registered documents in sync as code changes
